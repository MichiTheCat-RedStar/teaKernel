...
void print_char(char c) {
	if (c == '\n') {
		...
		if (cursor_position >= 2000) clear_screen(); // <--
		return;
	} else if (c == '\t') {
		unsigned int old_pos = cursor_position;
		cursor_position = cursor_position + (4 - (cursor_position % 4)); // 4 - размер таба
		while (old_pos < cursor_position && old_pos < 2000) {
			video[old_pos] = ' ' | (cursor_color << 8);
			old_pos++;
		}
		if (cursor_position >= 2000) clear_screen(); // <--
		return;
	...
	if (cursor_position >= 2000) clear_screen(); // В дальнейшем заменить на прокручивание, так же и там, где нарисовано "<--"
}
...
// Вывести строку с цветом без изменения основного цвета
void print_colored_string(const char* str, uint8_t color) {
	uint8_t old_color = cursor_color;
	set_color(color);
	print_string(str);
	set_color(old_color);
}
